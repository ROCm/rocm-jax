...
parser.add_argument(
    "--srcs",
    action="append",
    help="Source files (passed by jax_wheel macro, unused).",
)
parser.add_argument(
...
def prepare_rocm_plugin_wheel(wheel_sources_path: pathlib.Path, *, cpu, rocm_version, srcs):
    ...
    copy_from_srcs_or_runfiles(
        source_map, "pyproject.toml", wheel_sources_path
    )
    copy_from_srcs_or_runfiles(
        source_map, "setup.py", wheel_sources_path
    )
    copy_from_srcs_or_runfiles(
        source_map, "LICENSE.txt", wheel_sources_path
    )
