...
class OpsTest(PallasBaseTest):
    ...
    def test_func(self, func, x_shape_dtype:jax.ShapeDtypeStruct = jax.ShapeDtypeStruct((128,), jnp.float32), y_dtype:jnp.dtype = jnp.bool_):
        ...
        out = self.pallas_call(kernel, out_shape=jax.ShapeDtypeStruct(x_shape_dtype.shape, y_dtype))(x)
        # tol = 1e-6

        #self.assertAllClose(out, func(x), atol=tol, rtol=tol)
        self.assertArraysEqual(out, func(x))
        print(f"PASSED: {func.__name__}, {x_shape_dtype}")

if __name__ == "__main__":
    test = OpsTest()
    shape = (8,)
    """
    def print_isinf(dtype):
      x = jnp.array([float('nan'), float('inf'), float('-inf'), 1.0, 0.0, -0.0, -1.5, .3], dtype=dtype)
      print(x)
      print(dtype, jnp.isinf(x))

    print_isinf(jnp.float16)
    print_isinf(jnp.float8_e4m3b11fnuz)
    print_isinf(jnp.float8_e4m3fn)
    print_isinf(jnp.float8_e4m3fnuz)
    print_isinf(jnp.float8_e5m2)
    print_isinf(jnp.float8_e5m2fnuz)"""
